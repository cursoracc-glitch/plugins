...
namespace Oxide.Plugins
{
    ...
    public class Backpack : RustPlugin
    {
        ...
        void OnEntityDeath(BaseCombatEntity ent, HitInfo info)
        {
            ...
            {
                droppedItemContainer.playerName = $"Рюкзак игрока <color=#8e6874>{player.displayName}</color>";
        ...
        //private string TextInButton = lang.GetMessage("openbp", this, player.UserIDString);
        ...
        private void BackPackBox2(StorageContainer container, BasePlayer player)
        {
            var backpackSize = GetBackpackSize(player.UserIDString);
            if (backpackSize == 0) return;
            container.inventory.capacity = backpackSize;
        ...
        void ShowUIPlayer(BasePlayer player, BasePlayer target)
        {
            ...
            {
                RectTransform = { AnchorMin = "0.667 0.067", AnchorMax = "0.667 0.067", OffsetMin = "-30 -30", OffsetMax = "30 30" },
                Button = 
                {
                    Color = "1 1 1 0.04", 
                    Command = savedBackpacks.ContainsKey(target.userID) ? $"backpack_mainopen {target.userID}": "", 
            ...
            container.Add(new CuiElement
            {
                ...
                Components = 
                {
                    new CuiRawImageComponent { Png = m_FileManager.GetPng("backpackImage"), Color = "0.91 0.87 0.84 1.00" },
                    new CuiRectTransformComponent { AnchorMin = "0.005 0.1", AnchorMax = "0.18 0.9" }
                }
            });
            ...
            container.Add(new CuiElement
            {
                ...
                Components = 
                {
                    ...
                    {
                        Color=savedBackpacks.ContainsKey(target.userID) ? "0.91 0.87 0.84 1.00": "1.00 0.37 0.38 1.00", 
                        Text=savedBackpacks.ContainsKey(target.userID) ? $"ОТКРЫТЬ РЮКЗАК ИГРОКА": "РЮКЗАК ИГРОКА ПУСТ", 
       ...
       public bool BackpackHide(ulong userId)
        {
            ...
            if (otherPlayer != null) DrawUI(otherPlayer, true);
            else
                DrawUI(BasePlayer.FindByID(userId), true);
            return true;
        ...
        void OnUserPermissionGranted(string id, string permName)
        {
            if (permisions.ContainsKey(permName))
            {
                ...
                if (player != null) DrawUI(player, true);
            }
        }
        ...
        public List<BasePlayer> BackpackOpen = new List<BasePlayer>();
        
        [ConsoleCommand("backpackui.open")]
        private void ClickBackpackOpen(ConsoleSystem.Arg arg)
        {
            BasePlayer player = arg.Player();
            if (BackpackOpen.Contains(player))
            {
                BackpackOpen.Remove(player);
                DrawUI(player, false);
            }
            else
            {
                BackpackOpen.Add(player);
                DrawUI(player, true);
            }
        }
        
        void DrawUI(BasePlayer player, bool open)
        {
            if (!EnabledUI) return;
            if (!m_FileManager.IsFinished)
            {
                timer.Once(1f, () => DrawUI(player, true));
                return;
            }
            ...
            {
                Image = {Color= BackpackOpen.Contains(player) ? "0 0 0 0" : "1 1 1 0.03", Material= BackpackOpen.Contains(player) ? "" : "assets/content/ui/uibackgroundblur-ingamemenu.mat" }
                ,
                RectTransform = {AnchorMin = "0.667 0.067", AnchorMax = "0.667 0.067", OffsetMin = "-30 -30", OffsetMax = "30 30"}
                ,
            ...
            container.Add(new CuiButton
            {
                RectTransform =
                {
                    AnchorMin = BackpackOpen.Contains(player) ? "0 0" : "1.033335 0", 
                    AnchorMax = BackpackOpen.Contains(player) ? "0.2777777 0.2777777" : "1.311113 0.2777778"
                },
                Button = { Color = "1 1 1 0.03", Command = "backpackui.open", Material = "assets/content/ui/uibackgroundblur-ingamemenu.mat" },
                Text = { Text = BackpackOpen.Contains(player) ? ">" : "<", Align = TextAnchor.MiddleCenter, Font = "robotocondensed-regular.ttf", FontSize = 14 }
            }, "backpack.image");
            ...
            if (BackpackOpen.Contains(player))
            {
                container.Add(new CuiButton
                {
                    RectTransform =
                    {
                        AnchorMin = "0 0", AnchorMax = "0.2777777 0.2777777"
                    },
                    Button = { Color = "1 1 1 0.03", Command = "backpackui.open", Material = "assets/content/ui/uibackgroundblur-ingamemenu.mat" },
                    Text = { Text = BackpackOpen.Contains(player) ? ">" : "<", Align = TextAnchor.MiddleCenter, Font = "robotocondensed-regular.ttf", FontSize = 14 }
                }, "backpack.image");
            ...
            { 
                container.Add(new CuiButton
                {
                    RectTransform =
                    {
                        AnchorMin = "1.033335 0", AnchorMax = "1.311113 0.2777778"
                    },
                    Button = { Color = "1 1 1 0.03", Command = "backpackui.open", Material = "assets/content/ui/uibackgroundblur-ingamemenu.mat" },
                    Text = { Text = BackpackOpen.Contains(player) ? ">" : "<", Align = TextAnchor.MiddleCenter, Font = "robotocondensed-regular.ttf", FontSize = 14 }
                }, "backpack.image");
            ...
            string TextInButton = lang.GetMessage("openbp", this, player.UserIDString);
            container.Add(new CuiElement
            {
                Parent = "backpack.image",
                Components = {
                    new CuiTextComponent {
                        Color="1 1 1 0.5", Text = BackpackOpen.Contains(player) ? "" : TextInButton, FontSize = 11, Align = TextAnchor.MiddleCenter,
                    }
                    , new CuiRectTransformComponent {
