...
namespace Oxide.Plugins
{
    ...
    public class InfoPanel : RustPlugin
    {
					...
					{ "BackgroundBox", new PanelConfig
                        {
                            ...
                            Image = new PanelImageConfig
                            {
                                ...
                                Url = "https://i.imgur.com/Oe43E56.png",
                            },
                            Text = new PanelTextConfig
                            {
                                Align = TextAnchor.MiddleCenter,
                    ...
                    { "ChinookEvent", new PanelConfig
                        {
                            ...
                            Image = new PanelImageConfig
                            {
                                ...
                                Height = 0.8f,
                                Margin = "0 0.15 0.1 0.1",
                                Url = "http://i.imgur.com/xQ9fV6G.png",
                            },
                            PanelSettings = new Dictionary<string,object>
                            {
                                { "InactiveColor", "1 1 1 0.1" },
                                { "ActiveColor", "0.2 0.2 0.7 1" },
                            }
                        }
                    },
        ...
        void OnServerInitialized()
        {
			...
			BackgroundBox = new Backgrounds
            (
                Settings.BackgroundText,
				Settings.GetPanelSettingsValue("BackgroundBox", "Counter", 0)
            );

            Airplane = new AirplaneEvent();
            Helicopter = new HelicopterEvent();
            CH47 = new ChinookEvent();
            ...
            ActiveCH47 = UnityEngine.Object.FindObjectsOfType<CH47Helicopter>().ToList();

            if (ActiveCH47.Count > 0)
            {
                CheckCH47();
            }
            else
            {
                CH47.Refresh(storedData, PlayerPanels);
            }
        }
        ...
        private void OnEntitySpawned(BaseEntity Entity)
        {
            ...
            {
                ActiveCH47.Add((CH47Helicopter) Entity);
        ...
        private Backgrounds BackgroundBox;
        private List<string> BackgroundText = new List<string> { "vk.com/.......              rustkss.com" };
        private bool BackgroundBoxAvailable = true;


        public class Backgrounds
        {
            List<string> BackgroundText;
            private int Counter = 0;
            public Backgrounds(List<string> bgds, int Counter)
            {
                BackgroundText = bgds;
                this.Counter = 0;
        ...
        private ChinookEvent CH47;
        private List<CH47Helicopter> ActiveCH47;
        private bool CH47Timer = false;
        ...
        public void CheckCH47()
        {
            ActiveCH47.RemoveAll(p => !p.IsValid() || !p.gameObject.activeInHierarchy);

            if (ActiveCH47.Count > 0)
            {

                if (CH47.isActive == false)
                {
                    CH47.SetActivity(true);
                    CH47.Refresh(storedData, PlayerPanels);
                }

                CH47Timer = true;
                timer.In(5, CheckCH47);
                return;
            }

            CH47.SetActivity(false);
            CH47.Refresh(storedData, PlayerPanels);
            CH47Timer = false;
        }
        #endregion
            ...
            foreach (var Panel in PlayerPanels[player.UserIDString])
            {
                switch (Panel.Key)
                {
                    ...
                    case "MessageBoxText":
                        ...
                        break;
					case "BackgroundBoxText":
                        ((IPanelText) Panel.Value).Content = BackgroundBox.GetServer();
                        break;
                    ...
                    case "ChinookEventImage":
                        ((IPanelRawImage) Panel.Value).Color = CH47.ImageColor;
                        break;
                    case "CompassText":
